...
其中 `Ts`、三个符号方向、`tau`、当前平衡点 `target_pitch_deg` 最关键。质量、质心、转动惯量主要用于后续物理模型；当前 `rough_model` 已经使用 `r_wheel` 和 `l_com` 估算速度命令对车身角加速度的影响。

## WiFi 端测量 `tau`

当前 WiFi 调试页已经加入“轮速 Tau 测试”。它用于测底层速度环从目标轮速阶跃到实测轮速的响应时间，结果直接用于 `lqr_velocity_model_template.m` 里的 `tau`。

建议步骤：

1. 把车轮架空，确保轮子转动不会带动车身冲出去。
2. 打开 WiFi 调试页，找到“轮速 Tau 测试”。
3. 阶跃速度先用 `2.0 rad/s`，测试时长先用 `2500 ms`。
4. 点击“开始 Tau 测试”。程序会先关闭平衡，再给左右轮同一个目标轮速阶跃。
5. 页面会同时显示 `平均`、`左轮`、`右轮` 三组 Tau。每组 Tau 都是对应前进方向轮速到达 63.2% 阶跃幅值的时间。
6. 分别测 `+2.0 rad/s` 和 `-2.0 rad/s`，各测 3 次，去掉明显异常值后取平均。
7. 建模时先在 MATLAB 中填写 `tau = 平均Tau毫秒 / 1000`。如果左右轮差异明显，先检查左右轮速度环参数、摩擦和编码器速度符号。

符号约定：Tau 测试显示的左轮、右轮、平均值都统一成“车体前进方向”的速度。左轮使用原始实测速度，右轮实测速度取反后显示和计算；这只影响观测值，不改变右轮实际电机命令方向。也就是说，右轮当前前进方向正确这一点不会被 Tau 测试修改。
